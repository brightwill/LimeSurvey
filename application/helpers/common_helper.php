...
function getGroupList($gid,$surveyid)
{
	...
	foreach ($gidresult->readAll() as $gv)
	{
		...
		if ($gv['gid'] == $gid) {
			$groupselecter .= " selected='selected'"; $gvexist = 1;
		}
...
function getGroupDepsForConditions($sid,$depgid="all",$targgid="all",$indexby="by-depgid")
{
	...
	$sqltarggid="";
	if ($depgid != "all") {
		$depgid = sanitize_int($depgid); $sqldepgid="AND tq.gid=$depgid";
	}
	if ($targgid != "all") {
		$targgid = sanitize_int($targgid); $sqltarggid="AND tq2.gid=$targgid";
	}
...
function getQuestDepsForConditions($sid,$gid="all",$depqid="all",$targqid="all",$indexby="by-depqid", $searchscope="samegroup")
{
	...
	$sqlsearchscope="";
	if ($gid != "all") {
		$gid = sanitize_int($gid); $sqlgid="AND tq.gid=$gid";
	}
	if ($depqid != "all") {
		$depqid = sanitize_int($depqid); $sqldepqid="AND tq.qid=$depqid";
	}
	if ($targqid != "all") {
		$targqid = sanitize_int($targqid); $sqltargqid="AND tq2.qid=$targqid";
	}
...
function getUserGroupList($ugid=NULL,$outputformat='optionlist')
{
	...
	if (isset($groupnames))
	{
		foreach($groupnames as $gn)
		{
			...
			if ($gn['ugid'] == $ugid) {
				$selecter .= " selected='selected'"; $svexist = 1;
			}
